 ...
 * API Version: Returns a result object.
 */
...
function autofitColumns() {
  const result = apiAutofitColumns();
  if (!result.success) {
 ...
 * API Version: Accepts transactionId as parameter.
 */
...
function highlightTransactionRow() {
  ...
  if (response.getSelectedButton() == ui.Button.OK) {
    const result = apiHighlightTransaction(response.getResponseText());
    if (!result.success) {
